function  v = clampedspline( x,y,u,s )
n=length(x);
...
G=zeros(n,1);
G(1)=6*(((y(2)-y(1))/h(2))-s(1));
G(n)=6*(((y(n-1)-y(n))/h(n)) + s(2));
for i=2:n-1
...
fileid=fopen('output6.txt','w');
fprintf(fileid,'%s','Interpolated values of y* at given x* ');
fprintf(fileid,'\n');
fprintf(fileid,'%s','Clamped spline: ');
fprintf(fileid,'\n');
for i=1:size(u)
